...
sub new {
    my ( $package ) = @_;
    my $self;
    $self->{"ancestors"} = ["ringset", "isvitrite", "graph"];
...
sub make {
	...
	#vitrite
	print STDERR "IsVitrite";
	#bidir pipe
	my $pid = open2(*Reader, *Writer, "$BINDIR/rngs2ar3a.pl | $BINDIR/graphquench 2>/dev/null | $BINDIR/graphquench2 2>/dev/null | $BINDIR/graphquench4c 2>/dev/null" );
	Writer->autoflush(); # default here, actually
	#print Writer vitrite::ringset::to_rngs( $record->{ringset} );
	close Writer;
	@lines = <Reader>;
	close Reader;
    }
    ...
    #my $tmp = "/tmp/shape$$";
    #open OUT, ">$tmp";
    #print OUT vitrite::ringset::to_rngs( $record->{ringset} );
    #close OUT;
    #open IN, "$BINDIR/rngs2ar3a.pl < $tmp | $BINDIR/graphquench2 | $BINDIR/graphquench4c";
    #my @lines = <IN>;
    #close IN;
    #unlink $tmp;
    ...
    $record->{shape} = $shape;
    #print STDERR $record->{id}."SHAPE[$shape]";
    return $shape;
}
